...
-> export compact candidates.json
-> Agent/LLM writes decisions.json
-> apply decisions
...
3. Code builds `candidates.json` with title, source, URL, excerpt, source metadata, and history metadata.
4. Only send the best 30-50 candidates to the Agent/LLM.
5. Agent/LLM writes `decisions.json` instead of editing final outputs directly.
6. Code applies `decisions.json` into downstream draft/digest files.
7. Keep full text available for selected items when deeper interpretation is needed.

## Candidate and Decision Artifacts

Use explicit artifacts between code and Agent judgment:

- `output/candidates.json`: compact candidate snapshot generated by code.
- `output/decisions.json`: Agent/LLM decision file.
- `output/selected_news_draft.json`: code-applied draft for existing finalize/render scripts.

This makes runs resumable and debuggable. If Agent judgment fails, rerun from `candidates.json` without fetching all sources again.

Recommended decision item shape:

```json
{
  "candidate_id": "c0001",
  "tag": "AI设计",
  "brief_text": "A concise user-language explanation.",
  "final_score": 82,
  "topics": ["AI 产品落地"],
  "daily_priority": 86,
  "information_gain": 78,
